  ...
  const handleDeleteUser = async () => {
    ...
    if (confirm("هل أنت متأكد من رغبتك في حذف هذا الحساب نهائياً؟ هذا الإجراء لا يمكن التراجع عنه.")) {
      ...
      try {
        ...
        showToast("تم حذف بيانات المستخدم بنجاح (ملاحظة: لا يمكن حذف تسجيل الدخول من الواجهة)", "success");
        onBack();
      } catch(e: any) {
    ...
    const fetchProfile = () => {
      ...
      const unsubscribe = onSnapshot(doc(db, 'users_public', uid), async (snap) => {
        ...
        try {
          ...
          // Library count for badge
          const libQ = query(collection(db, 'library'), where('uid', '==', uid));
          const libSnap = await getDocs(libQ);
          setLibraryCount(libSnap.size);

          // Get the actual profiles for followers
          const followerUids = folsSnap.docs.map(d => d.data().followerUid);
          const followingUids = fingSnap.docs.map(d => d.data().followedUid);

          const getProfiles = async (uids: string[]) => {
            if (!uids.length) return [];
            // Firestore 'in' query has 10 item limit natively but since it's just viewing, 
            // we will fetch each via batched reads or getDocs. We'll use getDocs from 'users_public' manually.
            const uniqueUids = [...new Set(uids)].slice(0, 50); // limit 50 for simplicity
            const profiles: UserProfile[] = [];
            for (let chunk = 0; chunk < uniqueUids.length; chunk += 10) {
              const currentChunk = uniqueUids.slice(chunk, chunk + 10);
              const pq = query(collection(db, 'users_public'), where('uid', 'in', currentChunk));
              const ps = await getDocs(pq);
              ps.forEach(d => profiles.push(d.data() as UserProfile));
            }
            return profiles;
          };

          const [folsProfiles, fingProfiles] = await Promise.all([
            getProfiles(followerUids),
            getProfiles(followingUids)
          ]);

          setFollowers(folsProfiles);
          setFollowingList(fingProfiles);
...
function MainApp({ clerkUser, isClerkLoaded, clerkSignOut }: { clerkUser?: any, isClerkLoaded?: boolean, clerkSignOut?: any }) {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (u) => {
      ...
      if (u && !isClerkEnabled) {
        ...
        if (!userSnap.exists()) {
          const newProfile = {
            uid: u.uid,
            displayName: u.displayName || t('unknown_author', 'كاتب مجهول'),
            email: u.email || '',
            photoURL: u.photoURL || '',
            createdAt: serverTimestamp()
          };
          ...
          if (!publicSnap.exists()) {
            ...
            await setDoc(publicRef, {
              uid: u.uid,
              displayName: data?.displayName || u.displayName || t('unknown_author', 'كاتب مجهول'),
  ...
  useEffect(() => {
    if (isClerkEnabled && clerkUser) {
      const syncClerkProfile = async () => {
        ...
        // Generate a username: prefer clerk username, else derive from fullName or email
        ...
        if (!userSnap.exists()) {
          ...
          await setDoc(userRef, newProfile);
          await setDoc(doc(db, 'users_public', clerkUser.id), {
            uid: clerkUser.id,
            username,
            displayName: newProfile.displayName,
            photoURL: newProfile.photoURL,
          ...
          // If existing user has no username, add one
          const existingData = userSnap.data();
          if (!existingData.username) {
            const username = generateUsername();
            await updateDoc(userRef, { username });
            await setDoc(doc(db, 'users_public', clerkUser.id), { username }, { merge: true });
          }
        }
      };
      syncClerkProfile();
    }
  }, [isClerkEnabled, clerkUser]);

  useEffect(() => {
    if (!effectiveUserId) return;
    const unsubscribe = onSnapshot(doc(db, 'users', effectiveUserId), (snap) => {
      if (snap.exists()) {
  ...
  useEffect(() => {
    if (!effectiveUserId) return;
    const q = query(collection(db, 'follows'), where('followerUid', '==', effectiveUserId));
  ...
  useEffect(() => {
    if (!effectiveUserId) return;
    const q = query(collection(db, 'library'), where('uid', '==', effectiveUserId));
    const unsubscribe = onSnapshot(q, (snapshot) => {
  ...
  useEffect(() => {
    if (!effectiveUserId) return;
    const q = query(collection(db, 'readingProgress'), where('uid', '==', effectiveUserId));
    const unsubscribe = onSnapshot(q, (snapshot) => {
  ...
  useEffect(() => {
    if (!effectiveUserId) return;
    const q = query(collection(db, 'novels'), where('authorUid', '==', effectiveUserId), orderBy('updatedAt', 'desc'));
    const unsubscribe = onSnapshot(q, (snapshot) => {
  ...
  useEffect(() => {
    ...
    if (novelId) {
      const fetchNovel = async () => {
        ...
        if (novelSnap.exists()) {
          setSelectedNovel({ id: novelSnap.id, ...novelSnap.data() } as Novel);
          setView('novel');
        }
      };
      fetchNovel();
      ...
      const fetchByUsername = async () => {
        const q = query(collection(db, 'users_public'), where('username', '==', username.toLowerCase()), limit(1));
        const snap = await getDocs(q);
        if (!snap.empty) {
          setSelectedProfileUid(snap.docs[0].data().uid);
          setView('profile');
        }
      };
      fetchByUsername();
  ...
  if (!isClerkEnabled && user && !user.emailVerified && user.providerData.some(p => p.providerId === 'password') && verificationOnlyViews.includes(view)) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#FDFCFB] p-6 text-center">
          ...
          <div className="space-y-4">
            <button 
              onClick={() => {
                ...
                sendEmailVerification(user, actionCodeSettings).then(() => showToast(t('verification_resent', 'تم إعادة إرسال الرابط!')));
              }} 
  ...
  const incrementStat = async (novelId: string, stat: 'likesCount' | 'viewsCount' | 'sharesCount') => {
    ...
    try {
      ...
      if (stat === 'likesCount' || (stat === 'viewsCount' && effectiveUserId !== 'anonymous')) {
        const subColl = stat === 'likesCount' ? 'likes' : 'views';
        const docRef = doc(db, `novels/${novelId}/${subColl}`, effectiveUserId);
        ...
        await runTransaction(db, async (transaction) => {
          ...
          if (stat === 'likesCount') {
            ...
            // View - only increment if not already viewed
            if (!statsDoc.exists()) {
              transaction.set(docRef, {
                uid: effectiveUserId,
                createdAt: serverTimestamp()
        ...
        // For sharesCount or anonymous viewsCount
        if (stat === 'viewsCount') {
  ...
  const followAuthor = async (authorUid: string) => {
    ...
    if (!effectiveUserId) {
      // Guest: in-memory follow only — resets on page refresh
  ...
  const unfollowAuthor = async (authorUid: string) => {
    if (!effectiveUserId) {
      // Guest: remove from in-memory follows
  ...
  const addToLibrary = async (novelId: string) => {
    if (!effectiveUserId) {
      // Guest: in-memory library only — resets on page refresh
  ...
  const removeFromLibrary = async (novelId: string) => {
    if (!effectiveUserId) {
      // Guest: remove from in-memory library
      setGuestLibrary(prev => prev.filter(id => id !== novelId));
      return;
    }
    try {
      const q = query(collection(db, 'library'), where('uid', '==', effectiveUserId), where('novelId', '==', novelId));
      const snap = await getDocs(q);
  ...
  const updateReadingProgress = async (novelId: string, chapterId: string, chapterOrder: number) => {
    if (!effectiveUserId) return;
    try {
      const q = query(collection(db, 'readingProgress'), where('uid', '==', effectiveUserId), where('novelId', '==', novelId));
      const snap = await getDocs(q);
      if (snap.empty) {
  ...
  const handleSaveOnboarding = async (isSkipping = false) => {
    if (!isSkipping && !onboardingTeraBox.trim() && !effectiveUserId) return;
    if (!effectiveUserId || !userProfile) return;

    setIsSavingOnboarding(true);
    try {
      const userRef = doc(db, 'users', effectiveUserId);
      const publicRef = doc(db, 'users_public', effectiveUserId);
      ...
      handleFirestoreError(e, OperationType.UPDATE, `users/${effectiveUserId}`);
    }
    ...
    <div className="min-h-screen bg-[#FDFCFB] text-[#1A1A1A] pb-20 md:pb-0">
      ...
      <main className="mx-auto max-w-5xl px-4 sm:px-6 py-8 sm:py-12">
        <AnimatePresence mode="wait">
          ...
          {view === 'library' && (
            <LibraryView 
              library={effectiveUserId ? library : guestLibrary.map(id => ({ novelId: id, uid: '', addedAt: null as any }))} 
            ...
            <SettingsView 
              ...
              onUpdateProfile={async (data) => {
                ...
                try {
                  ...
                  if (photoURL && photoURL.startsWith('data:image/')) {
                    try {
                      ...
                      console.error("Compression failed", compressErr);
                                    ...
                                    // Define privateProfileData and encrypt email if it's being updated
                  const { teraboxLink, ...privateProfileData } = data;
                  const encryptedEmail = data.email ? encryptData(data.email) : undefined;
                  ...
                  console.log("Private profile updated:", updateData);
                  ...
                  const publicData = {
                    ...
                    teraboxLink: data.teraboxLink || '', // Store as plain text in public
                    links: data.links || [],
                    updatedAt: serverTimestamp()
                  };
                  ...
                  console.log("Public profile updating:", publicData);
                  await setDoc(publicRef, publicData, { merge: true });
                  console.log("Public profile updated successfully");

                  // Sync denormalized data in novels only if changed
                  if (data.displayName !== userProfile.displayName || photoURL !== userProfile.photoURL) {
                    const novelsQ = query(collection(db, 'novels'), where('authorUid', '==', effectiveUserId!));
                    ...
                    // Don't block the main save on this sync if it takes too long
                    Promise.all(syncPromises).catch(err => console.error("Sync failed", err));
                  }
                } catch (e) {
                  handleFirestoreError(e, OperationType.UPDATE, `users/${effectiveUserId!}`);
                }
  ...
  const handleGenerateShortSummary = async () => {
    ...
    try {
      ...
      if (summary) {
        ...
        // Update last chapter creation time in user profile
        if (userProfile?.uid) {
          try {
            await updateDoc(doc(db, 'users', userProfile.uid), {
              lastChapterCreatedAt: serverTimestamp()
            });
          } catch (e) {
            console.error("Error updating lastChapterCreatedAt", e);
          }
        }
      }
    } catch (e) {
...
const Reader = ({ 
  ...
  useEffect(() => {
    if (!currentUserId) return;
    const likeRef = doc(db, `novels/${novel.id}/likes`, currentUserId);
    const unsubscribe = onSnapshot(likeRef, (doc) => {
      setHasLiked(doc.exists());
    }, (error) => {
      handleFirestoreError(error, OperationType.GET, `novels/${novel.id}/likes/${currentUserId}`);
    });
    return () => unsubscribe();
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="flex items-center gap-2 rounded-full border border-purple-300 bg-purple-50 px-4 py-2 text-xs font-bold text-purple-700 transition-all hover:bg-purple-100 disabled:opacity-50"
          >
